...
- The library stays Node-targeted: the package root reaches `crypto` / `net` / `dns` and uses `Buffer`, so a Worker needs both `compatibility_flags = ["nodejs_compat"]` and `compatibility_date >= "2024-09-23"` (as it always has — the flag alone does not supply them), and a plain browser is still not a supported target. This change removes the codegen barrier, not the Node one.
