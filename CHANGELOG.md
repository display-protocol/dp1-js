...
- `RefManifest.i18n` is typed `Record<string, LocalizedMetadata>` instead of `Record<string, Metadata>`. The schema localizes only `title`, `description`, and `creditLine`, so the old type invited callers to put `artists`, `tags`, or `thumbnails` in a locale override, where they carry no defined meaning. A caller passing a full `Metadata` to `.i18n()` will now fail to compile; drop the non-localizable fields. This is also a parity fix: `dp1-go` has always modelled the field as `map[string]LocalizedMetadata` (`refmanifest/manifest.go`), with the same three fields. Runtime validation is unchanged — `LocalizedMetadata` does not set `additionalProperties: false`, so existing documents still validate.
