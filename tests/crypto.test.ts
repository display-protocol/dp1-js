  ...
  describe('signPlaylist and verifyPlaylistSignature integration', () => {
    ...
    describe('signDP1Playlist', () => {
      ...
      it('should create deterministic signatures for the same playlist', async () => {
        const signature1 = await signDP1Playlist(testPlaylist, keyPair.privateKey);
        const signature2 = await signDP1Playlist(testPlaylist, keyPair.privateKey);
      ...
      it('should handle complex playlist structures', async () => {
        ...
        const signature = await signDP1Playlist(complexPlaylist, keyPair.privateKey);
        expect(signature).toMatch(/^ed25519:0x[a-f0-9]{128}$/);
      });
    });
