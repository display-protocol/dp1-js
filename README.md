...
`npm run check:packaging` builds the package and runs `publint --strict` and `attw --pack .` over what would be published, so a broken `exports` map fails here rather than in a consumer's `tsc` (as [#30](https://github.com/display-protocol/dp1-js/issues/30) did). It runs in CI as the `packaging` job, and again in `publish.yml` before the package reaches npm.
