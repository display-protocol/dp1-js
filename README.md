...
- `PlaylistItemBuilder` carries the playlists-extension item fields: `.note()`, `.displayAt()`, and `.inlineManifest(manifest | RefManifestBuilder)`. Setting `note` or `displayAt` validates the item against the composed core + extension schema instead of core alone; an inline manifest is validated against the core ref-manifest schema.
...
One deviation is worth knowing: the spec's `playlist_item_with_extension.json` does not list `inlineManifest`, so validating a _single_ item through `ValidatePlaylistItemWithPlaylistsExtension` will not check a nested manifest. The embedded schemas mirror the spec byte-for-byte, so `PlaylistItemBuilder` closes the gap by validating the manifest with the core ref-manifest schema directly. Whole-playlist validation covers it either way, through the extension fragment.
